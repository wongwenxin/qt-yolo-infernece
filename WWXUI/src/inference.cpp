...
YOLO_INFERENCE_CORE::YOLO_INFERENCE_CORE() {
...
char *YOLO_INFERENCE_CORE::CreatSession(YOLO_INIT_PARAM &iparams) {
    char *Ret = RET_OK;
...
char *YOLO_INFERENCE_CORE::RunSession(cv::Mat iImg, vector<YOLO_RESULT> &oResult) {
#ifdef benchmark
...
template<typename N>
std::vector<YOLO_RESULT> YOLO_INFERENCE_CORE::TensorProcess(clock_t &starttime1, cv::Mat &iImg, N &blob, vector<int64_t> &inputNodeDims) {
//    1 创建 inputTensor
//    2 Run得到 outputTensor
//    3 处理 得到 信息

    vector<YOLO_RESULT> outResult;
    ...
    cout << outputNodeDims[1] << "  " << outputNodeDims[2] << endl;
    delete blob;
...
//            第二次筛选 只留下 (obj_conf * cls_conf) > conf_thres 的数据
            ...
            int classNum = outputNodeDims[2];
            int resultNum = outputNodeDims[1];
            float obj_confidence;
            float total_confidence;
            float *data = (float *) output;
            for (int i = 0; i < resultNum; i++) {
                float *classScores = data + 5;
                cv::Mat scores(1, this->classes.size(), CV_32F, classScores);
                cv::minMaxLoc(scores, 0, &maxConfidence, 0, &class_id);
                obj_confidence = data[4];
                total_confidence = obj_confidence * maxConfidence;
                if (obj_confidence > rectConfidenceThreshold) {
    ...
    for (int i = 0; i < nmsResult.size(); i++) {
        ...
        cout << "confidence: " << res.confidece << " class: " << res.classId << endl;
    }
#ifdef benchmark
...
//  1、前处理  2、推理
char *YOLO_INFERENCE_CORE::WarmUpSession() {
